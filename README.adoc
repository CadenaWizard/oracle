...
```
cd dlccryptlib_oracle && maturin develop && cd ..
```

(or with venv:)
```
cd dlccryptlib_oracle && VIRTUAL_ENV='../venv' ../venv/bin/maturin develop && cd ..
```

Set up the secret key used:
```sh
python3 ./dlccryptlib_oracle/save_secret.py --set --file secret.sec
```
